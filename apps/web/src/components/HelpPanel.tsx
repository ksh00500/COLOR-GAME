      ...
      <section className="help-panel" role="dialog" aria-modal="true" aria-labelledby={titleId} onMouseDown={(event) => event.stopPropagation()}>
        <div className="panel-heading">
          <div>
            ...
            <h2 id={titleId}>{t("연결을 완성하세요")}</h2>
          </div>
          <button className="icon-button" type="button" onClick={onClose} aria-label={t("도움말 닫기")}>×</button>
        </div>
        <ol className="rule-list">
          <li><strong>{t("공용 색상")}</strong><span>{t("세 색상은 양쪽 모두 자유롭게 사용합니다.")}</span></li>
          <li><strong>{t("마지막 한 수")}</strong><span>{t("연결을 완성한 플레이어가 점수를 얻습니다.")}</span></li>
