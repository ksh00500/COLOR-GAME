...
Easy는 모델 없이 단순 1수 알고리즘을 사용하고, Normal은 기존 온라인 22경기·626수로 학습한 경량 모델을 사용합니다. Hard는 실제 온라인 경기 데이터 30%와 로컬 자가대전 데이터 70%를 섞어 학습한 AlphaZero-lite 모델입니다.
...
```powershell
.\scripts\hard-ai\fetch-data.ps1
```
...
```powershell
.\scripts\hard-ai\train.ps1 -Preset smoke
```

실제 후보는 다음 중 하나로 학습합니다.

```powershell
.\scripts\hard-ai\train.ps1 -Preset quick
.\scripts\hard-ai\train.ps1 -Preset standard
.\scripts\hard-ai\train.ps1 -Preset deep
```
...
```powershell
.\scripts\hard-ai\train.ps1 -Preset standard -Resume
```
...
```powershell
.\scripts\hard-ai\status.ps1 -Preset standard
.\scripts\hard-ai\publish.ps1 -Preset standard
```

신규 Hard 모델은 Normal로 사용하는 626수 학습 모델과 선후공을 바꿔 평가합니다. 최소 20판, 무승부 제외 승률 55% 이상이어야 승격할 수 있습니다. 승격 명령은 `hard-alpha-model.json`을 교체하고 타입 검사, 전체 테스트, 빌드를 실행하지만 커밋과 EC2 배포는 하지 않습니다.

## 6. 배포 전 확인

```powershell
git diff -- packages/ai-engine/src/hard-alpha-model.json
git status --short
```
