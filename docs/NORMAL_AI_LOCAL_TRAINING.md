...
신규 모델은 Easy로 사용하는 626수 학습 모델과 선후공을 바꿔 평가합니다. 최소 20판, 무승부 제외 승률 55% 이상이어야 승격할 수 있습니다. 승격 명령은 `normal-alpha-model.json`을 교체하고 타입 검사, 전체 테스트, 빌드를 실행하지만 커밋과 EC2 배포는 하지 않습니다.
