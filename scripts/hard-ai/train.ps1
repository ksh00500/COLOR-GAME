...
if (-not (Test-Path $python)) { throw "먼저 .\scripts\hard-ai\setup.ps1 을 실행하세요." }
...
if (-not (Test-Path $humanData)) { throw "먼저 .\scripts\hard-ai\fetch-data.ps1 을 실행하세요." }
$arguments = @($trainer, "--preset", $Preset, "--output", $output, "--human-data", $humanData, "--easy-model", $easyModel)
if ($Resume) { $arguments += "--resume" }
& $python @arguments
if ($LASTEXITCODE -ne 0) { throw "Hard AI 학습이 실패했습니다." }
